...
import { createDevSession, createPromptTemplate, createRun, deletePromptTemplate, disconnectEtsy, getAuthStatus, getDashboardCatalog, getEtsyConnection, getRun, logout, publishEtsyListing, syncEtsyDraft, testConnection, updateConnection, updateListing, updateMockupTemplate, updatePromptTemplate, uploadAsset, type ApiListing, type DashboardCatalog, type EtsyListingMetadata, type SessionUser } from './client/api'

type View = 'dashboard' | 'workflows' | 'templates' | 'connections' | 'assets' | 'run'
type WizardStep = 'start' | 'source' | 'review' | 'prepare' | 'products' | 'destinations' | 'summary'
...
const wizardSteps: Array<{ id: WizardStep; label: string; number: string }> = [
  ...
  { id: 'prepare', label: 'Prepare artwork', number: '04' },
  { id: 'products', label: 'Products & mockups', number: '05' },
  { id: 'destinations', label: 'Destinations', number: '06' },
  { id: 'summary', label: 'Review & run', number: '07' },
]
...
function App() {
  ...
  const [resizeDesigns, setResizeDesigns] = useState(true)
  ...
  return (
    <div className="app-shell">
      ...
      {showWizard && <WizardModal
        ...
        resizeDesigns={resizeDesigns}
        setResizeDesigns={setResizeDesigns}
...
function RunMonitor({ catalog, onDashboard, runId }: { catalog: DashboardCatalog; onDashboard: () => void; runId?: string }) {
  ...
  return <><div className="page-wrap"><section className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" /> Run / {currentRun?.name ?? 'production workflow'}</div><h1>{currentRun?.name ?? 'Production'} is moving<span className="heading-period">.</span></h1><p>Every step runs independently. You can leave this page and come back anytime. {liveRun && `Status: ${liveRun.status}.`}</p></div><button className="outline-button" onClick={onDashboard}>← Back to overview</button></section><div className="run-hero"><div><span className="hero-kicker">Run status</span><h2>{progress}% <em>complete</em></h2><p>{viewCatalog.runs.find((run) => run.id === runId)?.detail ?? 'Live run details will appear as the worker processes this workflow.'}</p></div><div className="run-hero-progress"><div className="progress-label"><span>Overall progress</span><strong>{progress}%</strong></div><div className="run-progress-large"><span style={{ width: `${progress}%` }} /></div><small>{liveRun?.status === 'completed' ? 'Completed and ready for review.' : liveRun?.status === 'failed' ? 'The worker reported an error. Review the pipeline below.' : 'Processing in the background.'}</small></div></div><div className="run-layout"><section className="panel job-panel"><div className="panel-heading"><div><span className="section-kicker">Background jobs</span><h3>Pipeline progress</h3></div><span className="healthy-pill"><i /> 28 completed</span></div><div className="job-list">{(liveJobs.length ? liveJobs : (viewCatalog.runs.find((run) => run.id === runId)?.jobs ?? [])).map((job, index) => [job.stepName, job.status, job.status, job.errorLog]).map(([label, detail, state, errorLog], index) => <div className="job-row" key={label}><span className={`job-state ${state}`}>{state === 'complete' || state === 'completed' ? '✓' : state === 'running' ? '⋯' : state === 'failed' ? '!' : String(index + 1).padStart(2, '0')}</span><div><strong>{label}</strong><small>{errorLog || detail}</small></div><span className={`job-label ${state}`}>{state === 'complete' || state === 'completed' ? 'Complete' : state === 'running' ? 'Processing' : state === 'failed' ? 'Failed' : 'Queued'}</span>{state === 'running' && <button className="text-button">Retry</button>}</div>)}</div></section><section className="panel listings-panel"><div className="panel-heading"><div><span className="section-kicker">Preview</span><h3>Listing drafts</h3></div><button className="text-button">Open all →</button></div>{runListings.slice(0, 4).map((listing) => <button className="listing-row listing-row-button" key={listing.id} onClick={() => { const raw = rawListings.find((item) => item.id === listing.id); if (raw?.marketplace === 'etsy') setListingEditor(raw) }}><div className="listing-art" style={{ background: listing.background, color: listing.accent }}>{listing.icon}</div><div><strong>{listing.title}</strong><small>{listing.type} · {listing.tags.slice(0, 2).join(' · ')}</small></div><span className={`status-badge ${listing.status === 'draft' || listing.status === 'ready' || listing.status === 'published' ? 'ready' : 'draft'}`}>{listing.status}</span></button>)}<div className="run-assets"><div className="subheading-row"><div><strong>Generated assets</strong><small>{liveAssets.length} files persisted for this run</small></div>{isCompleted && <a className="dark-button download-button" href={`/api/runs/${runId}/export.zip`} download>Download structured ZIP <span>↓</span></a>}</div><div className="run-asset-grid">{liveAssets.slice(0, 6).map((asset) => <a className="run-asset-card" href={asset.url} target="_blank" rel="noreferrer" key={asset.id}><img src={asset.url} alt={asset.name} /><small>{asset.name}</small></a>)}</div></div></section></div></div>{listingEditor && <EtsyListingEditor listing={listingEditor} mockups={liveAssets.filter((asset) => asset.type === 'mockup')} onClose={() => setListingEditor(null)} onChanged={(listing) => { setListingEditor(listing); setRawListings((items) => items.map((item) => item.id === listing.id ? listing : item)) }} />}</>
}

function EtsyListingEditor({ listing, mockups, onClose, onChanged }: { listing: ApiListing; mockups: Array<{ id: string; name: string; url: string }>; onClose: () => void; onChanged: (listing: ApiListing) => void }) {
  const [form, setForm] = useState(listing.metadata as EtsyListingMetadata)
  const [message, setMessage] = useState(listing.lastError ?? '')
  const busy = listing.status === 'syncing' || listing.status === 'publishing'
  const selected = form.imageIds.map((id) => mockups.find((mockup) => mockup.id === id)).filter(Boolean) as typeof mockups
  const setNumber = (key: keyof EtsyListingMetadata, value: string) => setForm({ ...form, [key]: Number(value) })
  const save = async () => { try { const response = await updateListing(listing.id, form); onChanged(response.listing); setMessage('Preview saved.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to save') } }
  const sync = async () => { try { const response = await syncEtsyDraft(listing.id); onChanged(response.listing); setMessage('Draft sync queued. This view will update as the worker runs.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to sync') } }
  const publish = async () => { if (!window.confirm('Activate this Etsy listing? Etsy may charge a listing fee. This action publishes the external draft.')) return; try { const response = await publishEtsyListing(listing.id); onChanged(response.listing); setMessage('Publication queued.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to publish') } }
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="simple-modal etsy-listing-modal"><div className="modal-heading"><div><span className="section-kicker">Etsy listing · {listing.status}</span><h2>Review the external draft.</h2><p>Edit locally, sync an Etsy draft, then publish through a separate fee-bearing confirmation.</p></div><button className="close-modal" onClick={onClose}>×</button></div><div className="modal-form"><label>Title<input maxLength={140} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label><label>Description<textarea rows={6} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><label>Tags<input value={form.tags.join(', ')} onChange={(event) => setForm({ ...form, tags: event.target.value.split(',').map((tag) => tag.trim()).filter(Boolean).slice(0, 13) })} /></label><div className="etsy-fields"><label>Price<input type="number" min="0.01" step="0.01" value={form.price} onChange={(event) => setNumber('price', event.target.value)} /></label><label>Quantity<input type="number" min="1" value={form.quantity} onChange={(event) => setNumber('quantity', event.target.value)} /></label><label>Taxonomy ID<input type="number" value={form.taxonomyId} onChange={(event) => setNumber('taxonomyId', event.target.value)} /></label><label>Shipping profile ID<input type="number" value={form.shippingProfileId} onChange={(event) => setNumber('shippingProfileId', event.target.value)} /></label><label>Readiness profile ID<input type="number" value={form.readinessStateId} onChange={(event) => setNumber('readinessStateId', event.target.value)} /></label><label>SKU<input value={form.sku} onChange={(event) => setForm({ ...form, sku: event.target.value })} /></label></div><div><strong className="field-title">Selected images · first is main</strong><div className="etsy-image-order">{selected.map((mockup, index) => <div key={mockup.id}><img src={mockup.url} alt={mockup.name} /><small>{index === 0 ? 'Main image' : `Image ${index + 1}`}</small><span><button disabled={index === 0} onClick={() => { const ids = [...form.imageIds]; [ids[index - 1], ids[index]] = [ids[index], ids[index - 1]]; setForm({ ...form, imageIds: ids }) }}>←</button><button onClick={() => setForm({ ...form, imageIds: form.imageIds.filter((id) => id !== mockup.id) })}>×</button></span></div>)}</div></div>{message && <div className="info-note"><span>✧</span><span>{message}</span></div>}{listing.externalUrl && <a href={listing.externalUrl} target="_blank" rel="noreferrer">Open Etsy listing ↗</a>}</div><div className="modal-actions"><button className="outline-button" disabled={busy || listing.status === 'published'} onClick={() => void save()}>Save preview</button><button className="outline-button" disabled={busy || listing.status === 'published'} onClick={() => void sync()}>{listing.externalId ? 'Update Etsy draft' : 'Create Etsy draft'}</button><span /><button className="primary-button" disabled={listing.status !== 'draft'} onClick={() => void publish()}>Publish on Etsy ↗</button></div></div></div>
}
...
interface WizardProps {
  ...
  resizeDesigns: boolean
  setResizeDesigns: (value: boolean) => void
  maxDimension: number
...
function WizardModal(props: WizardProps) {
  const { step, currentStepIndex, source, setSource, selectedProvider, setSelectedProvider, selectedModel, setSelectedModel, selectedStyle, setSelectedStyle, includeText, setIncludeText, negativePrompt, setNegativePrompt, availableConnections, prompt, setPrompt, designCount, setDesignCount, selectedDesigns, setSelectedDesigns, uploadedFiles, onFiles, selectedProducts, toggleProduct, selectedTemplates, availableTemplates, toggleTemplate, destinations, setDestinations, removeBackground, setRemoveBackground, resizeDesigns, setResizeDesigns, maxDimension, setMaxDimension, products, mockups, onClose, onNext, onBack, onLaunch } = props
...
function SourceStep({ source, setSource, selectedProvider, setSelectedProvider, selectedModel, setSelectedModel, selectedStyle, setSelectedStyle, includeText, setIncludeText, negativePrompt, setNegativePrompt, availableConnections, prompt, setPrompt, designCount, setDesignCount, onFiles }: Pick<WizardProps, 'source' | 'setSource' | 'selectedProvider' | 'setSelectedProvider' | 'selectedModel' | 'setSelectedModel' | 'selectedStyle' | 'setSelectedStyle' | 'includeText' | 'setIncludeText' | 'negativePrompt' | 'setNegativePrompt' | 'availableConnections' | 'prompt' | 'setPrompt' | 'designCount' | 'setDesignCount' | 'onFiles'>) { return <div className="wizard-form-step"><span className="big-step-mark">02</span><span className="section-kicker">Design source</span><h2>Bring the idea to<br /><em>life.</em></h2><p>Start with a theme and let your configured provider create a consistent collection — or bring your own artwork.</p><div className="source-toggle"><button className={source === 'ai' ? 'selected' : ''} onClick={() => setSource('ai')}><span>✦</span> Generate with AI</button><button className={source === 'upload' ? 'selected' : ''} onClick={() => setSource('upload')}><span>↥</span> Upload existing</button></div>{source === 'ai' ? <div className="form-stack"><label>Collection theme<textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} /></label><div className="style-picker"><div className="subheading-row"><div><strong>Visual style</strong><small>Choose the art direction for the generated designs and mockups.</small></div><span className="style-selected">{imageStyleOptions.find((option) => option.id === selectedStyle)?.label}</span></div><div className="style-option-grid">{imageStyleOptions.map((option) => <button type="button" className={`style-option ${selectedStyle === option.id ? 'selected' : ''}`} key={option.id} onClick={() => setSelectedStyle(option.id)}><strong>{option.label}</strong><small>{option.description}</small></button>)}</div></div><div className="prompt-controls"><label className="toggle-label"><span><strong>Allow text in artwork</strong><small>{includeText ? 'The model may render intentional lettering from your prompt.' : 'Discourage all text and lettering in designs and mockups.'}</small></span><input type="checkbox" checked={includeText} onChange={(event) => setIncludeText(event.target.checked)} /></label>{!includeText && <label>Additional things to avoid<textarea className="negative-prompt-input" value={negativePrompt} onChange={(event) => setNegativePrompt(event.target.value)} placeholder="e.g. skulls, neon colors, busy backgrounds" rows={2} /></label>}</div><div className="form-columns"><label>Number of designs<div className="number-control"><button onClick={() => setDesignCount(Math.max(1, designCount - 1))}>−</button><strong>{designCount}</strong><button onClick={() => setDesignCount(Math.min(100, designCount + 1))}>＋</button></div></label><label>Image provider<select value={selectedProvider} onChange={(event) => setSelectedProvider(event.target.value)}>{availableConnections.filter((connection) => connection.id !== 'storage' && connection.capability !== 'mockup' && connection.capability !== 'transform' && connection.enabled && connection.configured).map((connection) => <option value={connection.id} key={connection.id}>{connection.name}</option>)}<option value="mock">Local mock</option></select>{availableConnections.find((connection) => connection.id === selectedProvider)?.models.length ? <select className="model-input" value={selectedModel} onChange={(event) => setSelectedModel(event.target.value)}>{availableConnections.find((connection) => connection.id === selectedProvider)?.models.map((modelOption) => <option value={modelOption} key={modelOption}>{modelOption}</option>)}</select> : <input className="model-input" value={selectedModel} onChange={(event) => setSelectedModel(event.target.value)} placeholder="Default model" />}</label></div><div className="info-note"><span>✧</span><span>Using <strong>{selectedProvider === 'fal' ? 'Fal.ai · Flux Schnell' : selectedProvider === 'openrouter' ? 'OpenRouter image generation' : selectedProvider === 'huggingface' ? 'Hugging Face · Inference API' : selectedProvider === 'ollama' ? `Ollama · ${selectedModel || 'x/flux2-klein'}` : 'Local mock provider'}</strong>. The provider can be changed later without changing your workflow.</span></div></div> : <div className="upload-zone"><span>↥</span><strong>Drop your designs here</strong><small>PNG, JPG or WEBP · up to 100 files</small><button className="outline-button" onClick={() => document.getElementById('design-upload')?.click()}>Browse files</button><input id="design-upload" type="file" accept="image/png,image/jpeg,image/webp" multiple hidden onChange={(event) => onFiles(event.target.files)} /></div>}</div> }
...
function PrepareStep({ removeBackground, setRemoveBackground, resizeDesigns, setResizeDesigns, maxDimension, setMaxDimension }: Pick<WizardProps, 'removeBackground' | 'setRemoveBackground' | 'resizeDesigns' | 'setResizeDesigns' | 'maxDimension' | 'setMaxDimension'>) { return <div className="wizard-form-step prepare-step"><span className="big-step-mark">04</span><span className="section-kicker">Prepare artwork</span><h2>Clean up<br /><em>before mockups.</em></h2><p>Remove backgrounds and cap artwork dimensions so templates render consistently.</p><div className="form-stack"><label className="toggle-label"><span><strong>Remove background</strong><small>{removeBackground ? 'Artwork backgrounds will be removed before fan-out.' : 'Keep original backgrounds as-is.'}</small></span><input type="checkbox" checked={removeBackground} onChange={(event) => setRemoveBackground(event.target.checked)} /></label><label className="toggle-label"><span><strong>Resize designs</strong><small>{resizeDesigns ? `Cap longest side at ${maxDimension}px.` : 'Skip resizing.'}</small></span><input type="checkbox" checked={resizeDesigns} onChange={(event) => setResizeDesigns(event.target.checked)} /></label>{resizeDesigns && <label>Max dimension<div className="number-control"><button onClick={() => setMaxDimension(Math.max(256, maxDimension - 100))}>−</button><strong>{maxDimension}</strong><button onClick={() => setMaxDimension(Math.min(8192, maxDimension + 100))}>＋</button></div></label>}</div><div className="info-note"><span>✧</span><span>Original files are never overwritten. This creates prepared copies for mockup generation.</span></div></div> }
...
function SummaryStep({ source, selectedProvider, selectedModel, selectedStyle, includeText, prompt, designCount, removeBackground, resizeDesigns, maxDimension, products, mockups, selectedProducts, destinations }: { source: 'ai' | 'upload'; selectedProvider: string; selectedModel: string; selectedStyle: ImageStyle; includeText: boolean; prompt: string; designCount: number; removeBackground: boolean; resizeDesigns: boolean; maxDimension: number; products: number; mockups: number; selectedProducts: ProductType[]; destinations: string[] }) { return <div className="wizard-form-step summary-step"><span className="big-step-mark">07</span><span className="section-kicker">Ready when you are</span><h2>Your production line<br /><em>looks good.</em></h2><p>We’ll create the workflow, snapshot this configuration as version 01, and queue each unit of work independently.</p><div className="summary-flow"><div className="flow-block"><span>01</span><strong>{source === 'ai' ? 'Generate designs' : 'Import designs'}</strong><small>{source === 'ai' ? `${designCount} variations · ${selectedProvider === 'fal' ? 'Fal.ai' : selectedProvider === 'openrouter' ? 'OpenRouter' : selectedProvider === 'huggingface' ? 'Hugging Face' : selectedProvider === 'ollama' ? `Ollama (${selectedModel || 'x/flux2-klein'})` : 'Local mock'}` : `${designCount} selected assets`}</small></div><i>→</i><div className="flow-block"><span>02</span><strong>Prepare artwork</strong><small>{removeBackground ? 'Background removal' : 'No background removal'} · {resizeDesigns ? `Max ${maxDimension}px` : 'No resize'}</small></div><i>→</i><div className="flow-block"><span>03</span><strong>Fan out variants</strong><small>{selectedProducts.length} product types · {products} total</small></div><i>→</i><div className="flow-block"><span>04</span><strong>Render mockups</strong><small>{mockups} template outputs</small></div><i>→</i><div className="flow-block"><span>05</span><strong>Deliver</strong><small>{destinations.length} destinations</small></div></div><div className="summary-prompt"><span>Prompt direction</span><strong>{source === 'ai' ? `“${prompt}”` : 'Uploaded design collection'}</strong><small>{source === 'ai' ? `${imageStyleOptions.find((option) => option.id === selectedStyle)?.label} · ${includeText ? 'Text allowed' : 'No text'}` : 'Existing artwork will be preserved'}</small></div><div className="approval-row"><span className="pulse-dot" /><span>Design review checkpoint is enabled</span><span className="approval-policy">Review before mockups</span></div></div> }
