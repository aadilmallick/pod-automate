...
import { createDevSession, createRun, getAuthStatus, getDashboardCatalog, getRun, logout, uploadAsset, type DashboardCatalog, type SessionUser } from './client/api'
...
const emptyCatalog: DashboardCatalog = { stats: { workflows: 0, assets: 0, readyListings: 0 }, connections: [], runs: [], assets: [], templates: [], listings: [] }
...
function App() {
  ...
  async function launchWorkflow() {
    ...
    try {
      const response = await createRun({ name: 'Funny cat collection', prompt, count: source === 'ai' ? designCount : selectedDesigns.length, products: selectedProducts, destinations, provider: selectedProvider, templates: selectedTemplates, assetIds: source === 'upload' ? selectedDesigns : [] })
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeView === 'dashboard' && <Dashboard catalog={activeCatalog} user={sessionUser} onCreate={openWizard} launched={launched} launchError={launchError} onViewRun={(id) => { if (id) setRunId(id); setActiveView('run') }} />}
        ...
        {activeView === 'connections' && <Connections catalog={activeCatalog} />}
        {activeView === 'assets' && <Assets catalog={activeCatalog} onCreate={openWizard} />}
...
function Dashboard({ catalog, user, onCreate, launched, launchError, onViewRun }: { catalog: DashboardCatalog; user: SessionUser | null; onCreate: () => void; launched: boolean; launchError?: string; onViewRun: (id?: string) => void }) {
...
function Connections({ catalog }: { catalog: DashboardCatalog }) {
  const connections = catalog.connections.map((connection) => ({ ...connection, status: connection.configured ? 'Connected' : 'Not connected', color: connection.id === 'openrouter' ? 'violet' : connection.id === 'fal' ? 'dark' : 'blue', mark: connection.id === 'openrouter' ? '✦' : connection.id === 'fal' ? 'f' : '◉' }))
...
function Assets({ catalog, onCreate }: { catalog: DashboardCatalog; onCreate: () => void }) {
  return <div className="page-wrap"><section className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" /> Workspace / asset library</div><h1>Asset library<span className="heading-period">.</span></h1><p>Your source designs, with provenance attached.</p></div><button className="outline-button" onClick={onCreate}>＋ Upload assets</button></section><div className="asset-toolbar"><div className="search-field">⌕ <span>Search assets</span></div><div className="filter-pills"><button className="filter-active">All assets</button><button>Generated</button><button>Uploaded</button><button>Mockups</button></div></div><div className="asset-grid">{catalog.assets.map((asset) => <div className="asset-card" key={asset.id}><div className="asset-art" style={{ background: asset.background, color: asset.accent }}><img src={asset.url} alt={asset.name} /><small>{asset.name}</small></div><div className="asset-card-meta"><strong>{asset.name}</strong><small>{asset.type} · {asset.contentType}</small><button className="mini-more">•••</button></div></div>)}</div></div>
}
