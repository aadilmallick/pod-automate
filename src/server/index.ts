...
import { assets, jobs, marketplaceConnections, marketplaceListings, marketplaceOauthSessions, mockups, mockupTemplates, productVariants, promptTemplates, runs, users, workflows, workspaces, workspaceConnections } from './db/schema'
...
const providerModels: Record<string, string[]> = { fal: [config.FAL_IMAGE_MODEL], openrouter: [config.OPENROUTER_IMAGE_MODEL], huggingface: [config.HUGGINGFACE_IMAGE_MODEL, 'black-forest-labs/FLUX.2-klein-9B'], gemini: [config.GEMINI_MOCKUP_MODEL], 'bg-removal': ['auto', 'sharp', 'imgly', 'none'], mock: ['local-mock'] }
