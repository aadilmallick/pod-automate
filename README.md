...
- Ollama uses the official `ollama` JavaScript SDK. It discovers the local service at `OLLAMA_BASE_URL` (default: `http://localhost:11434`) and lists installed image models whose names start with `x/`; the workflow only accepts models returned by that discovery. Pull a model first with `ollama pull x/flux2-klein` or `ollama pull x/flux2-klein:9b`. The Compose API/worker services use `http://host.docker.internal:11434` by default so they can reach Ollama running on the host; set `OLLAMA_DOCKER_BASE_URL` if needed. The Compose override is intentionally separate from `OLLAMA_BASE_URL`, so a host-side `.env` value of `http://localhost:11434` cannot break container networking. If you use the optional Compose Ollama service instead, start it with `docker compose --profile ollama up -d ollama` and set `OLLAMA_DOCKER_BASE_URL=http://ollama:11434`.
